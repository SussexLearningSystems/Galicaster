...
class DDP(Thread):
  ...
  def subscribed(self, subscription):
    ...
    if me:
      self.client.update('rooms', {'_id': self.id}, {'displayName': self.displayName, 'audio': audio, 'ip': self.ip}, callback=self.update_callback)
    else:
  ...
  def changed(self, collection, id, fields, cleared):
    ...
    level = int((float(me['audio']['rearMicBoost']['value']['left']) / float(me['audio']['rearMicBoost']['limits']['max'])) * 100)
  ...
  def control_values(self, mixer):
    ...
    controls['value'] = {'left': int(round((float(left) / 100) * maximum)), 'right': int(round((float(right) / 100) * maximum))}
    return controls
