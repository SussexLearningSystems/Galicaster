...
def init():
    ...
    hourly_wake = conf.get_boolean('sussexscreensaver',
                                   'hourly_wake') or False
    logger.debug('hourly_wake set to %s', hourly_wake)

    hourly_wake_from = conf.get_int('sussexscreensaver',
                                    'hourly_wake_from') or 8
    logger.debug('hourly_wake_from set to %i', hourly_wake_from)

    hourly_wake_to = conf.get_int('sussexscreensaver',
                                  'hourly_wake_to') or 17
    logger.debug('hourly_wake_to set to %i', hourly_wake_to)

    hourly_wake_minute = conf.get_int('sussexscreensaver',
